...
function getsabtekharidteatr(){
  ...
  $sql2 = "SELECT * FROM teatrwendsday where id ='$info'";
  $result2 = $conn->query($sql2);
  $conn->query("SET CHARSET SET 'utf8'");

  while ($information = mysqli_fetch_array($result2)) {
    $teatr_name_id = $information['teatr'];
    $teatr_sal = $information['salon'];
    $sql3 = "SELECT * FROM teatr where teatrid ='$teatr_name_id'";
    $result3 = $conn->query($sql3);
    $conn->query("SET CHARSET SET 'utf8'");
    while ($res=mysqli_fetch_array($result3)) {
      $teatr_name=$res['name'];
      $teatr_img=$res['image1'];

    }
    $time_info = $information['saat'];
    $price_info = $information['fi'];
  }
  }
   ...
   $cinema_address = $cinema['address'];
      echo"<div class='row text-justify'>
          <div class='col-md-3 pr-4'>
              <img src='$teatr_img' class='img-fluid rounded img-thumbnail' style='height: 300px;'>
          </div>
          <div class='col-md-5 '>
              <div class='pt-3 pr-0'>
                  <h5><strong>
